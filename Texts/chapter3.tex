...
\begin{equation}
	...
	x \in \{1\} \cup [a, \, b]
\end{equation}
in order for the Markov process to generate states according to the corresponding probability distribution $ P $. To cover the whole parameter space so that ergodicity is satisfied, it is more than sufficient to consider four types of transitions with corresponding transition probabilities $ P(1|1) $, $ P(1|x) $, $ P(x|1) $ and $ P(x|x') $. Here it is understood that $ x, x' \in [a, \, b] $ so that these states in parameter space belong to the first order term. The Markov process described is illustrated in figure \ref{fig:example2}.
...
\begin{figure}[H]
	\begin{fmffile}{FMFFILEPropagatorChangeOfTransferredMomentum}
		...
		\end{empheq}
	\end{fmffile}%
	\caption{The momentum transferred via an internal phonon propagator is updated. In order to obey momentum conservation at every vertex, electronic propagators beneath the phonon arc absorb the momentum difference.}
	\label{fig:NORMALcotm}
\end{figure}

This update procedure is similar to that of the external momentum, the main difference of course being that the magnitude of momentum transferred via a phonon propagator is updated rather than the external momentum. In order to achieve this, a $ \tilde \Dt^{(0)} $ is randomly selected and assigned a new momentum magnitude $ q_k \rightarrow q'_k $ while keeping the direction fixed. In order to conserve momentum at the two vertices connected to the phonon propagator, the $ \Gt^{(0)} $'s located beneath the phonon arc should absorb the momentum difference $ \vec q_k - \vec q'_k $. This is illustrated in figure \ref{fig:NORMALcotm}. Since the $ \tilde \Dt^{(0)}(\vec q, \tau) $'s does not depend on the magnitude $ q = | \vec q | $, one has that
\begin{equation}
	\begin{split}
...
It can be shown that the first order self-energy diagram $ \Sigma^{*(1)}(\vec p, \tau) $ is proportional to $ 1 / \sqrt \tau $ and thus diverges when $ \tau \rightarrow 0 $. It is not possible to capture such a behavior using a histogram since the second derivative of the function to be approximated is too large. To show this, consider the mean value of a function $ f(x) $ on an interval of width $ \Delta x $ centered at $ x_i $ and expanding using the small parameter $ \Delta x $, i.e.
\begin{equation}
	\begin{split}
...
\end{equation}
which has the proportionality $ \Delta \Gt(\vec p, \omega \rightarrow \infty) \propto \omega^{-5/2} $ because $ \Sigma^*(\vec p, \omega \rightarrow \infty) \propto \omega^{-1/2} $ and thus falls of fast enough. This $ \omega $ dependence will also have the consequence that any high frequency noise present in $ \Sigma^*(\vec p, \tau) $ will be greatly suppressed when transforming back $ \Delta \Gt $ to $ \tau $-space. Then, since also the inverse Fourier transform is linear, it follows that
\begin{equation}
			...
			\begin{tikzpicture}[baseline={([yshift=-.5ex]current bounding box.center)}]
				\node {
					\FDframe{
						\begin{fmfgraph*}(11, 6)
							\fmfbottom{b1,b2}
							\fmf{heavy, foreground=(0,,0,,1)}{b1,b2}
							\fmf{dashes, left}{b1,b2}
							\fmfdot{b1,b2}
						\end{fmfgraph*}%
					}
				...
				\node {
					\FDframe{
						\begin{fmfgraph*}(11, 1)					
							\fmfstraight
							\fmfbottom{i,o}
							\fmf{heavy, foreground=(0.8,,0.2,,1)}{i,o}
						\end{fmfgraph*}%
					}%
				};
			\end{tikzpicture} \\
...
\begin{fmffile}{FMFFILEthirdIterationG}
	...
	\end{empheq}
\end{fmffile}%
Already at this point, a wide range of diagrams contribute to $ \Gt_3 $. However, even if one continues this iterative process, a skeleton diagram in which the phonon propagators intersect will never be encountered in the series.
...
Nine different simulations for an increasing value of the maximum diagram order $ n_\text{max} $ have been carried out using the discretization $ \Delta \tau = 0.02 $, $ \Delta p = 0.02 $, $ p_\text{max} = 10 $ and $ \tau_\text{max} = 60 $. Depicted in figures \ref{fig:boldNmax1}-\ref{fig:boldNmax9} are the resulting $ S_k $'s as well as the corresponding $ \Gt_k $'s. The true value of the proper self-energy $ \Sigma^* $ and the Greens function $ \Gt $ are also outlined for comparison.
...
Unfortunately there was only four iterations used in the bold-line scheme, which according to figure \ref{fig:EandZvsK} was not enough in order to reach a steady state value for $ E_0 $ and $ Z_0 $.  This steady state could nevertheless not have been very far away, a fifth or perhaps a sixth iteration should probably have been sufficient for the trends to level out. Additionally it would also have been necessary to simulate using larger $ n_\text{max} $ in pursuance of convergence with the true $ E_0 $ and $ Z_0 $. But since this would have required a monstrous amount of computation time in order for the error bars to shrink to a satisfactory size it was not carried out.
