...
\begin{fmffile}{FMFFILEsecondIterationGandS}
	...
	\end{empheq}
\end{fmffile}%
By substituting with $ \Gt_1 $ in terms of the diagrams series (\ref{eq:BoldG1}) into the diagram series of $ \Gt_2 $, this series, expressed in terms of bare constituents, becomes
...
\begin{itemize}
	...
	\item \todo{Quickly mention how to calculate $ S_i $ normalization wise.}
\end{itemize}
...
\begin{itemize}
	\item \todo{Talk in general about how the update procedures are changed.}
	\item \todo{When times are being sampled, they are taken from a new distribution which is an exponential fit in time of $ \Gt_k(\vec p_i, \tau_i) $.}
	\item \todo{$ \Gt_k(\vec p_i, \tau_j) = \Gt^{(0)}(\vec p_i, \tau_j) + \Delta \Gt_k(\vec p_i, \tau_j) = e^{- \xi_k(\vec p_i, \tau_j) \, \tau_j} \Gt^{(0)}(\vec p_i, \tau_j) $. The $ \xi_k (\vec p_i, \tau_j) = \xi_{k,i,j} $ is a discrete function. Introduce then the "imaginary" phase $ \varphi(\vec p, \tau) = - \xi_k(\vec p_l, \tau_m) \, \tau $ where $ p_l - \Delta p/2 \leq p \leq p_l + \Delta p/2 $ and $ \tau_m - \Delta \tau /2 \leq \tau \leq \tau_m + \Delta \tau /2 $.}
\end{itemize}
...
\begin{itemize}
	\item \todo{Mention the computation time per bold-line iteration for each of these simulations.}
	\item \todo{Discuss and show a picture of how $ E_0 $ and $ Z_0 $ are extracted.}
\end{itemize}
...
\begin{itemize}
	\item \todo{$ E_0 $ and $ Z_0 $ vs bold-line iteration $ k $ plot for each $ n_\text{max} $.}
	\item \todo{Mention that $ N_\text{max} = 9 $ is extremely noisy, thus the large error bars.}
	\item \todo{Discuss why there is a discrepancy. Might be a discretization error or perhaps not all diagrams are taken into account. Might also need higher orders since it might be the case that the contribution of a diagram is proportional to its order $ n $. Would need an immense computation time for higher order, so that is not done. Also one should perform more bold iterations.}
\end{itemize}

\section{Numerical Results}

This thesis would not be complete without...

\begin{itemize}
	\item \todo{$ E_0 $ and $ Z_0 $ vs $ p $ plot at $ \alpha = 1 $}
	\item \todo{$ E_0 $ and $ Z_0 $ vs $ \alpha $ plot at $ p = 0 $}
	\item \todo{Table of the data plotted}
\end{itemize}
