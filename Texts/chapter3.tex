...
The following simulations were obtained using $ \Delta \tau = 0.005 $, $ \Delta p = 0.02 $, $ p_\text{max} = 10 $ and various $ \tau_\text{max} $ depending on the importance of the long time physics \question{(!?)}. 

\begin{itemize}
	\item \todo{What parameters has been used}
\end{itemize}
...
\begin{itemize}
	...
	\item \todo{Discuss and show a picture of how $ E_0 $ and $ Z_0 $ are extracted. Minimize $ \log \Gt - \log \Gt_\text{fit} $ for long times.}
\end{itemize}
...
\begin{itemize}
	\item \todo{Add to the plots perturbative analytical approaches.}
