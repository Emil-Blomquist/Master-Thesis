...
Here follow some results obtained when simulating $ \Gt $ using the bare scheme update procedures described above. However, the external momentum $ \vec p $ has been fixed during these simulations in order to not spread the statistics out over external parameters for which the data is not shown.

\begin{figure}[H]
	\centering
 	\includegraphics[width=\textwidth, ]{{"Images/Plots/Gt/Gt_a1_p0_bare"}.pdf}
	\caption{The acquired value of $ \Gt(p = 0, \, \tau) $ after 80 minutes of core time using the chemical potential $ \mu = -1.1 $ and interaction parameter $ \alpha = 1 $. Here $ \tau_{max} = 40 $ and $ \Delta \tau = 0.02 $.}
	\label{fig:GtOfA1P0}
\end{figure}
...
\begin{figure}[H]
	\centering
 	\includegraphics[width=\textwidth, ]{{"Images/Plots/Gt/Gt_a1_p15_bare"}.pdf}
		\caption{The acquired value of $ \Gt(p = 1.5, \, \tau) $ after 80 minutes of core time using the chemical potential $ \mu = -0.25 $ and interaction parameter $ \alpha = 1 $. Here $ \tau_{max} = 40 $ and $ \Delta \tau = 0.02 $.}
	\label{fig:GtOfA1P1.5}
\end{figure}

\begin{figure}[H]
	\centering
 	\includegraphics[width=\textwidth, ]{{"Images/Plots/Gt/Gt_a2_p0_bare"}.pdf}
		\caption{The acquired value of $ \Gt(p = 0, \, \tau) $ after 80 minutes of core time using the chemical potential $ \mu = -2.2 $ and interaction parameter $ \alpha = 2 $. Here $ \tau_{max} = 40 $ and $ \Delta \tau = 0.02 $.}
	\label{fig:GtOfA2P0}
\end{figure}
...
\end{table}
\todo{same precision for all cells!!}
...
\todo{To avoid any difficulty regarding...}
...
Because $ \Sigma^*(\vec p, \omega \rightarrow \infty) \propto 1/\sqrt \omega $ ... \question{make sure about this}.
...
\todo{
\begin{itemize}
	\item Singular part of diagram
	\item $ 1/\omega^2 $ and shit
	\item High resolution needed
	\item Show $ \Gt $ obtained via Dyson
\end{itemize}
}
