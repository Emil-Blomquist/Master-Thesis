...
\begin{itemize}
	...
	\item \todo{Perhaps quickly mention the Heisenberg picture here?}
\end{itemize}
...
\end{equation}
Here the subscripts of $ \vec P_\text{ir} $ and $ \omega_\text{ir} $ have been dropped since the ultra violet quantities no longer appear in our theory. The reasons to why one prefer to work with $ \vec B $ rather than $ \vec P_\text{ir} $ will be evident later. With this our now non-divergent Hamiltonian becomes,
...
The first task to be dealt with is to symmetrize the product $ b_{\vec q}^\dagger b_{\vec q} $. To this point, $ b_{\vec q}^\dagger $ and $ b_{\vec q} $ have been c-valued variables so that the order in which they appeared such a product does not matter. However, due to a nonzero commutation relation when promoted to operators, this will no longer be the case. Hence, in order to not favor any of the two possible orderings, they are chosen to contribute equally, i.e. $ b^\dagger_{\vec q} b_{\vec q} = (b^\dagger_{\vec q} b_{\vec q} + b_{\vec q} b^\dagger_{\vec q})/2 $.


Next up, using \cite{superfluidStatesOfMatter}
\begin{equation}
	\dot q = \frac{\delta H}{\delta p}
	\; , \quad
	\dot p = - \frac{\delta H}{\delta q}
\end{equation}
together with (\ref{eq:driven_harmonic_oscillator}) and (\ref{eq:Hamiltonian_raw}) it is easy to verify that $ \vec P $ and $ \gamma \dot{ \vec P} $ or equivalent, $ (b^\dagger_{\vec q} + b_{\vec q})/\sqrt{2 \gamma \omega} $ and $ i (b^\dagger_{\vec q} - b_{\vec q}) \sqrt{\gamma \omega / 2} $ for each $ \vec q $, are conjugate variables \question{(how do we arrive at the final set of conjugate variables? Using some sort of transformation perhaps? Try find in the book by Egor.)}.  By promoting to operators $ b_{\vec q} \rightarrow \hat b_{\vec q} $, $ \vec r_\text{e} \rightarrow \hat{\vec r}_\text{e} $, $ \vec p_\text{e} \rightarrow \hat{\vec p}_\text{e} $ and imposing Bose statistics \question{(How do we know this? spinless?)} onto the conjugate variables,

\begin{equation}
	i \delta_{\vec q, \vec k}
	= \frac{i}{2} [\hat b^\dagger_{\vec q} + \hat b_{\vec q}, \, \hat b^\dagger_{\vec k} - \hat b_{\vec k}]/2 \question{\text{two "2"'s}}
\end{equation}

\question{This follow from the poisson brackets + canonical variables!?
% https://en.wikipedia.org/wiki/Poisson_bracket
}

\question{Perhaps $ V $ is important for the equation above?}
...
\question{There should also pop out a $ 1/V $ from the free electron term of the Hamiltonian somehow!}
...
\begin{itemize}
\item
\motivation{Comparing with (4.18) in \cite{electronsInLatticeFields}, it seems as if the coordinates used are not dimensionless.}

\item
\motivation{Comparing with (8.6) in \cite{RichardFeynman}: ... Also we shall sometimes use the notation

\begin{equation}
	\sum_{\vec k} = \int \frac{\diff^3 k}{(2 \pi)^3} \,...
\end{equation}
}

\item
\motivation{Comparing with (8.5) in \cite{RichardFeynman}: The Hamiltonian of the free electron and phonos is

\begin{equation}
	\hat H = \frac{\hat p^2}{2M} + \hbar \omega \int \frac{\diff^3 k}{(2 \pi)^3} \hat a^\dagger_{\vec k} \hat a_{\vec k}
\end{equation}
}
...
\todo{Number of diagrams at each order $ \tfrac{(2n)!}{n!2^n} $}

\todo{Since there is no preferred direction of the external momentum $ \vec p $, the Green's function will depend only upon the magnitude $ p = | \vec p | $, i.e. $ \Gt = \Gt(\alpha, \mu, p, \tau) $.}

It is obvious that theses diagrams only contribute if the times are ordered chronologically, i.e. $ 0 = \tau_0 \le \tau_1 \le \tau_2 \le \dots \le \tau_{2n - 1} = \tau $. Because if this was not the case at least one propagator would propagate backwards in time and contribute with a factor of zero. Thus it is only meaningful to carry out the integrals such that this chronologization constraint is fulfilled.
