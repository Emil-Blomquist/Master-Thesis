...
\end{equation}
and thus constitutes a set of canonical field coordinates. By employing a restricted canonical transformation \cite{ClassicalMechanics} $ q_i = q_i(\phi, \pi), \, p_i = p_i (\phi, \pi) $, the canonical coordinates may be expressed in terms of $ b_{\vec q}^\dagger $ and $ b_{\vec q} $ as
\begin{equation}
	q_{\vec k} = \sqrt{\frac{1}{2 \omega \gamma}} \left[ b^\dagger_{\vec k} + b_{\vec k} \right]
	\; , \quad
	p_{\vec k} = i \sqrt{\frac{\gamma}{2 \omega}} \left[ b^\dagger_{\vec k} + b_{\vec k} \right] \,.
\end{equation}
This is pretty straight forward to verify since such a transformation satisfy
\begin{equation}
	\todo{\cdots}
\end{equation}
